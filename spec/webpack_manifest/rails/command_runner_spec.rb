  ...
  describe '#run' do
    ...
    context 'when the command exit successfully' do
      ...
      it 'sends messages to the logger' do
        expect(logger).to receive(:info).with('Executing echo hi && echo fail > /dev/stderr')
    ...
    context 'when the command exit unsuccessfully' do
      ...
      it 'sends messages to the logger' do
        expect(logger).to receive(:info).with('Executing echo hi && echo fail > /dev/stderr && false')
